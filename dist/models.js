 ...
 */
export function getModelTimeout(model) {
    const entry = MODEL_LOOKUP.get(model);
    if (entry)
 ...
 */
export function getStallTimeout(model) {
    const entry = MODEL_LOOKUP.get(model);
    if (entry)
        return entry.stallTimeoutMs;
    const lower = (model || "").toLowerCase();
    if (lower.includes("opus"))
        return 120000;
    if (lower.includes("haiku"))
...
export function normalizeModelName(model) {
    if (!model)
        return CANONICAL_IDS.sonnet;
    const lower = model.toLowerCase();
    if (lower.includes("opus"))
        return CANONICAL_IDS.opus;
    if (lower.includes("sonnet"))
        return CANONICAL_IDS.sonnet;
    if (lower.includes("haiku"))
        return CANONICAL_IDS.haiku;
    return model;
}
/**
